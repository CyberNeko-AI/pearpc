...
void gcard_osi(int cpu)
{
	...
	case 28: {
		...
		if (gDisplay->changeResolution(*chr)) {
			...
			gcard_set_mode(*chr);
            PPC_DIAG_TRACE("[GCARD] set_vmode: OK %dx%dx%d\n", chr->width, chr->height, chr->bytesPerPixel*8);
		} else {
			ppc_cpu_set_gpr(cpu, 3, 1);
            IO_GRAPHIC_WARN("set_vmode: FAILED (changeResolution)\n");
		}
		return;
	}
	case 29: {
		...
		if (vmode > (int)gGraphicModes->count() || vmode < 0) {
			ppc_cpu_set_gpr(cpu, 3, 1);
			if (osi29count < 100)
                PPC_DIAG_TRACE("[GCARD] get_vmode_info[%d]: vmode=%d(req=%d) → FAIL (out of range, count=%d)\n",
		...
		if (osi29count < 100)
            PPC_DIAG_TRACE("[GCARD] get_vmode_info[%d]: vmode=%d(req=%d) depth_mode=%d → %dx%dx%d scanline=%d nmodes=%d\n",
            ...
            PPC_DIAG_TRACE("[GCARD] get_vmode_info: suppressing further traces (100 reached)\n");
		osi29count++;
		return;
	}
	case 31:
